  ...
  return (
    <div className="flex-1 overflow-y-auto bg-slate-50 p-12 animate-in fade-in duration-700">
      <div className="max-w-7xl mx-auto space-y-12">
        {/* Welcome Section */}
        <div className="flex justify-between items-end bg-white rounded-[3.5rem] p-16 shadow-2xl relative overflow-hidden border border-slate-100 group">
          ...
          <div className="space-y-6 relative z-10 max-w-2xl">
            <div className="inline-flex items-center px-4 py-2 bg-indigo-50 rounded-full border border-indigo-100 mb-2">
              <span className="w-2 h-2 rounded-full bg-indigo-500 animate-pulse mr-2"></span>
              <span className="text-[10px] font-black text-indigo-600 uppercase tracking-widest">Neural Link v4.5 Ready</span>
            </div>
            <h1 className="text-6xl font-black text-slate-900 tracking-tighter leading-tight">
              Welcome back, <span className="text-indigo-600">{ownerInfo.name.split(' ')[0]}</span>.
            </h1>
            ...
            <div className="flex space-x-4 pt-4">
              <button 
                ...
                className="px-8 py-4 bg-slate-900 text-white rounded-2xl font-black text-xs uppercase tracking-widest shadow-xl hover:bg-black transition-all transform active:scale-95"
              >
                Go to Boards
              </button>
              <button 
                onClick={() => onSelectPage('movie-studio')}
                className="px-8 py-4 bg-white border-2 border-slate-100 text-slate-900 rounded-2xl font-black text-xs uppercase tracking-widest hover:border-indigo-400 hover:text-indigo-600 transition-all shadow-sm"
              >
                New Production
              </button>
            </div>
          ...
          <div className="hidden lg:flex items-center space-x-10 relative z-10">
             <div className="text-center">
                ...
                <p className="text-4xl font-black text-indigo-600 tracking-tighter">98%</p>
             </div>
           ...
           <div className="col-span-12 lg:col-span-8 space-y-8">
              <div className="bg-white rounded-[3rem] p-10 shadow-sm border border-slate-100 space-y-8">
                 <div className="flex justify-between items-center px-2">
                    <h3 className="text-xl font-black text-slate-900 tracking-tight">Recent Mission Streams</h3>
                    <button onClick={() => onSelectPage('portfolio')} className="text-[10px] font-black text-indigo-600 uppercase tracking-widest hover:underline">View Portfolio →</button>
                 </div>
                 <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    {recentBoards.map(board => (
                       <div key={board.id} onClick={() => onSelectBoard(board.id)} className="p-8 bg-slate-50 hover:bg-white rounded-[2.5rem] border-2 border-transparent hover:border-indigo-100 transition-all group cursor-pointer shadow-inner hover:shadow-xl">
                          ...
                          <h4 className="text-lg font-black text-slate-900 mb-1 truncate">{board.name}</h4>
                          <p className="text-xs text-slate-500 font-medium line-clamp-2 leading-relaxed mb-6">{board.description}</p>
                          <div className="h-1.5 w-full bg-slate-200 rounded-full overflow-hidden">
                             <div className="h-full bg-indigo-500 w-2/3"></div>
                          </div>
                       </div>
                    ))}
                    <button className="p-8 border-4 border-dashed border-slate-100 rounded-[2.5rem] flex flex-col items-center justify-center space-y-4 text-slate-400 hover:border-indigo-200 hover:text-indigo-600 hover:bg-indigo-50/20 transition-all group">
                       <div className="w-12 h-12 rounded-full border-4 border-slate-100 flex items-center justify-center group-hover:bg-indigo-600 group-hover:border-indigo-600 group-hover:text-white transition-all shadow-md">
              ...
              <div className="bg-slate-900 rounded-[3rem] p-10 shadow-2xl text-white space-y-8 relative overflow-hidden">
           ...
           <div className="col-span-12 lg:col-span-4 space-y-8">
              <div className="bg-white rounded-[3rem] p-10 shadow-sm border border-slate-100 space-y-8 h-full flex flex-col">
                 <div className="flex justify-between items-center px-2 shrink-0">
                    <h3 className="text-xl font-black text-slate-900 tracking-tight">Neural Pulse</h3>
                    <span className="text-[10px] font-black bg-rose-50 text-rose-500 px-3 py-1 rounded-full uppercase border border-rose-100">Live</span>
                 </div>
                 ...
                 <div className="space-y-6 flex-1">
                    {[
                       ...
                       <div key={idx} className="flex items-start space-x-4 p-4 rounded-3xl hover:bg-slate-50 transition-colors group/signal cursor-pointer">
                          ...
                          <div className="flex-1 min-w-0">
                             <div className="flex justify-between items-center mb-0.5">
                                <h4 className="font-black text-slate-900 text-sm truncate">{signal.title}</h4>
                                <span className="text-[8px] font-bold text-slate-400 uppercase">{signal.time}</span>
                             </div>
                             <p className="text-[10px] text-slate-500 font-bold uppercase tracking-tight truncate">{signal.meta}</p>
                          </div>
                       </div>
                    ))}
                 </div>
                 ...
                 <div className="pt-8 border-t border-slate-50 mt-auto">
                    <button onClick={() => onSelectPage('connections')} className="w-full py-5 bg-slate-50 hover:bg-slate-100 rounded-2xl text-[10px] font-black uppercase tracking-widest text-slate-600 transition-all">Clear Signals</button>
                 </div>
              </div>
           </div>
        </div>
