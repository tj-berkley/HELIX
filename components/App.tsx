...
const App: React.FC = () => {
  ...
  const handleAIGenerate = async (prompt: string, options?: BoardGenerationOptions) => {
    ...
    try {
      const ai = await generateBoardFromPrompt(prompt, options);
      if (ai) {
        const newBoard: Board = {
          id: `b-${Date.now()}`,
          name: ai.name || 'AI Generated Board',
          description: ai.description || 'Generated by Gemini AI',
          groups: (ai.groups || []).map((g: any, idx: number) => ({
            id: `g-${idx}-${Date.now()}`,
            name: g.name,
        ...
        <header className="h-14 border-b border-white/5 bg-[#0c0e12]/80 backdrop-blur-md flex items-center justify-between px-8 shrink-0 z-40">
           <div className="flex items-center space-x-6 flex-1">
              <div className="flex items-center space-x-2 text-slate-400">
                 ...
                 <Icons.ChevronRight />
                 <span className="text-xs font-black text-slate-100">{activeBoard?.name || 'Dashboard'}</span>
              </div>
