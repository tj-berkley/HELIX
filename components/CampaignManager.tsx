          ...
          <div className="space-y-4">
            ...
            <div className="flex bg-white border border-slate-200 p-1 rounded-2xl shadow-sm">
               {['active', 'templates'].map(t => (
                 <button key={t} onClick={() => setActiveTab(t as any)} className={`px-6 py-2 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all ${activeTab === t ? 'bg-indigo-600 text-white shadow-lg' : 'text-slate-400 hover:text-slate-600'}`}>{t === 'active' ? 'Live' : 'Templates'}</button>
               ))}
            </div>
          </div>
